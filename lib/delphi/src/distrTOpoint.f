...
c here we have a spherical distribution
	    ...
	    if (charge.ne.0.0) then
	        ...
	        qplus=qplus+charge
	        cqplus(1)=cqplus(1)+charge*xb(1)
	        cqplus(2)=cqplus(2)+charge*xb(2)
	        cqplus(3)=cqplus(3)+charge*xb(3)
	      else
	        qmin=qmin+charge
	        cqmin(1)=cqmin(1)+charge*xb(1)
	        cqmin(2)=cqmin(2)+charge*xb(2)
	        cqmin(3)=cqmin(3)+charge*xb(3)
	      end if
	    end if
...
c here we have a cylindric distribution
		...
		if (charge.ne.0.0) then
            if (charge.gt.0.0) then
	        qplus=qplus+charge
	        cqplus(1)=cqplus(1)+charge*(xa(1)+xb(1))*0.5
	        cqplus(2)=cqplus(2)+charge*(xa(2)+xb(2))*0.5
	        cqplus(3)=cqplus(3)+charge*(xa(3)+xb(3))*0.5
	      else
	        qmin=qmin+charge
	        cqmin(1)=cqmin(1)+charge*(xa(1)+xb(1))*0.5
	        cqmin(2)=cqmin(2)+charge*(xa(2)+xb(2))*0.5
	        cqmin(3)=cqmin(3)+charge*(xa(3)+xb(3))*0.5
	      end if
	    end if
          if (strtmp(11:12).eq.' v') then
c           uniform in the volume
...
c è da verificare assegnazione qcharge!!!
	        qplus=qplus+charge
	        cqplus(1)=cqplus(1)+charge*(2*xa(1)+xb(1))/3
	        cqplus(2)=cqplus(2)+charge*(2*xa(2)+xb(2))/3
	        cqplus(3)=cqplus(3)+charge*(2*xa(3)+xb(3))/3
	      else
	        qmin=qmin+charge
	        cqmin(1)=cqmin(1)+charge*(2*xa(1)+xb(1))/3
	        cqmin(2)=cqmin(2)+charge*(2*xa(2)+xb(2))/3
	        cqmin(3)=cqmin(3)+charge*(2*xa(3)+xb(3))/3
	      end if
	    end if
...
c       assegno momenti di carica
	    qnet=qnet+charge
		if (charge.ne.0.0) then
            if (charge.gt.0.0) then
	        qplus=qplus+charge
	        cqplus(1)=cqplus(1)+charge*xm(1)
	        cqplus(2)=cqplus(2)+charge*xm(2)
	        cqplus(3)=cqplus(3)+charge*xm(3)
	      else
	        qmin=qmin+charge
	        cqmin(1)=cqmin(1)+charge*xm(1)
	        cqmin(2)=cqmin(2)+charge*xm(2)
	        cqmin(3)=cqmin(3)+charge*xm(3)
	      end if
	    end if
c 
...
c c'è qualche discrepanza!! almeno con il manuale
          read(strtmp(27:80),*)xa,xb,radius
          call basisortho(xa,xb,xu,xv,xw,modul)
	    ...
	    if (charge.ne.0.0) then
            if (charge.gt.0.0) then
	        qplus=qplus+charge
	        cqplus(1)=cqplus(1)+charge*xa(1)
	        cqplus(2)=cqplus(2)+charge*xa(2)
	        cqplus(3)=cqplus(3)+charge*xa(3)
	      else
	        qmin=qmin+charge
	        cqmin(1)=cqmin(1)+charge*xa(1)
	        cqmin(2)=cqmin(2)+charge*xa(2)
	        cqmin(3)=cqmin(3)+charge*xa(3)
	      end if
	    end if
...
c here we have a rectangular surface distribution
		...
		if (charge.ne.0.0) then
            if (charge.gt.0.0) then
	        qplus=qplus+charge
	        cqplus(1)=cqplus(1)+charge*(xc(1)+xd(1))*0.5
	        cqplus(2)=cqplus(2)+charge*(xc(2)+xd(2))*0.5
	        cqplus(3)=cqplus(3)+charge*(xc(3)+xd(3))*0.5
	      else
	        qmin=qmin+charge
	        cqmin(1)=cqmin(1)+charge*(xc(1)+xd(1))*0.5
	        cqmin(2)=cqmin(2)+charge*(xc(2)+xd(2))*0.5
	        cqmin(3)=cqmin(3)+charge*(xc(3)+xd(3))*0.5
	      end if
	    end if
