	subroutine getatm(fname,nam1,ifrm,idfrm,iatinf,iatrad,iatcrg,atot,
     &  nmedia,nobject,ndistr,repsin,epkt,realsiz)
...
c b+++++++++++++++++++
        ...
        integer iatmmed(natmax),nmedia,objecttype,realsiz           
...
c e+++++++++++++++++++
	...
	integer atot
c
	i_atpos=memalloc(i_atpos,realsiz,3*natmax)
	i_rad3=memalloc(i_rad3,realsiz,natmax)
	i_chrgv4=memalloc(i_chrgv4,realsiz,natmax)
	i_atinf=memalloc(i_atinf,1,15*natmax)
c b+++++++++++++++++++
        i_medeps=memalloc(i_medeps,realsiz,nmediamax)
        i_dataobject=memalloc(i_dataobject,4,96*nobjectmax*2)
...
c     jmao, for mcce all fort.13 assumed unformatted
      ifrm=.false.
c
...
c
	if((idfrm.eq.2).or.(idfrm.eq.1).or.(idfrm.eq.4)) then
	iatinf=.true.
	iatrad=.true.
	iatcrg=.true.
	read(9,204,end=4000,err=4000)line
...
c e++++++++++++++++++++++
	  ...
	  if(idfrm.eq.2) then
            ...
            xfield=line(55:80)
            read(crdstr,205)xo
c            read(xfield,'(2f12.7)')radx,crgx
...
c
	i_atpos=memalloc(i_atpos,realsiz,3*atot)
	i_rad3=memalloc(i_rad3,realsiz,atot)
	i_chrgv4=memalloc(i_chrgv4,realsiz,atot)
	i_atinf=memalloc(i_atinf,1,15*atot)
c b+++++++++++++++++++
        do ii=1,nobject
...
c         objects are considered like atoms in this case
        ...
        i_medeps=memalloc(i_medeps,realsiz,nmedia+1)
        i_dataobject=memalloc(i_dataobject,4,96*nobject*2)
