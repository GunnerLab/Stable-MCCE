...
cWWW	if ((i.eq.17).and.(j.eq.6).and.(k.eq.13)) then
c            write(6,*) itmp,iepsmp(i,j,k,3)
c          end if
c         iaca=1
...
c dis= distance to atom surface
	...
	if(iac.eq.0)then
         if (debug) then
	      ...
	      write(6,*)"midpoint",j," in position [A]",s1,s2,s3
...
c debug++++++++++++++++++++++++
      if (debug) then
        open(52,file='bgpPoi.log',form='formatted')
c      real coordinates
	   write(52,*)igrid,ibnum
          do iiii=1,ibnum
            write(52,*)iiii,scspos(3*iiii-2),
     &            scspos(3*iiii-1),scspos(3*iiii)
          end do
        close (52)
	endif
c end debug+++++++++++++++++++
