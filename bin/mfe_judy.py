...
def get_mfe(env, residue, mfe_ph, mfe_xts, mfe_pwcut, pKa):
    ...
    report_lines.append("=================================\n")
    report_lines.append("Terms         meV  \n")
    report_lines.append("---------------------------------\n")
    report_lines.append("ebkb     %8.2f\n" % (dG_point.epol / mev2Kcal))
    report_lines.append("dsol     %8.2f\n" % (dG_point.dsolv / mev2Kcal))
    if mfe_xts:
        report_lines.append("-TS      %8.2f\n" % (-dG_point.TS/mev2Kcal))
    else:
        report_lines.append("-TS      %8.2f%8.2f\n" % (0.0 / mev2Kcal, 0.0))
    report_lines.append("residues %8.2f\n" % (dG_point.mfe_total / mev2Kcal))
    report_lines.append("*********************************\n")
    report_lines.append("TOTAL    %8.2f%8.9s\n" % (dG_point.G / mev2Kcal, "  sum_crg"))
    report_lines.append("*********************************\n")
    for i in range(len(dG_point.mfe)):
        if abs(dG_point.mfe[i] / ph2Kcal) > mfe_pwcut:
            ion_state = titration.residues[i].point_crg
            report_lines.append("%-9s%8.2f%8.2f\n" % (titration.residues[i].resid, dG_point.mfe[i] / mev2Kcal, ion_state))
    report_lines.append("=================================\n")
